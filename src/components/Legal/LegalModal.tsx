...
import { ShieldCheck, Scale, FileText, X, AlertTriangle } from 'lucide-react';
import { SoundFxService } from '../../services/soundFxService';

interface LegalModalProps {
  isOpen: boolean;
  initialTab?: 'privacy' | 'terms' | 'imprint';
  onClose: () => void;
}
...
export const LegalModal: React.FC<LegalModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'privacy' | 'terms' | 'imprint'>(initialTab);
      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }} className="glow-text-cyan">
            ...
            <div>
              ...
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                Der Wegweiser • DSGVO, StVO-Sicherheit & Impressum
              </p>
            </div>
          </div>
          <button
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '6px' }}>
          ...
          <button
            ...
            style={{ padding: '8px', fontSize: '0.75rem', justifyContent: 'center' }}
          >
            <ShieldCheck size={14} /> Datenschutz (DSGVO)
          </button>
          ...
          <button
            ...
            style={{ padding: '8px', fontSize: '0.75rem', justifyContent: 'center' }}
          >
            <Scale size={14} /> Impressum & Lizenzen
          </button>
        </div>
        ...
        {activeTab === 'terms' && (
          <div style={{ fontSize: '0.82rem', lineHeight: '1.5', color: '#e2e8f0', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <h4 style={{ color: 'var(--accent-cyan)', fontWeight: 'bold' }}>§ 4 Tokens & Gamification</h4>
            <p>
              Gesammelte oder erworbene In-App Tokens begründen keinen Anspruch auf Barauszahlung oder Rückvergütung.
            </p>
          </div>
        )}
        ...
        {activeTab === 'privacy' && (
          <div style={{ fontSize: '0.82rem', lineHeight: '1.5', color: '#e2e8f0', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <p>
              * <strong>GPS & Navigation (Art. 6 Abs. 1 lit. b DSGVO)</strong>: Standortdaten werden zur Live-Navigation, Kursausrichtung und GPX-Aufzeichnung im Vordergrund und Hintergrund verarbeitet. Es erfolgt kein dauerhaftes Bewegungsprofiling auf zentralen Servern.<br />
            ...
            <p>
              Du hast jederzeit das Recht auf Auskunft, Berichtigung, Löschung und Datenübertragbarkeit deiner gespeicherten Daten. Kontaktiere uns einfach formlos per E-Mail.
            </p>
          </div>
        )}
        ...
        {activeTab === 'imprint' && (
          <div style={{ fontSize: '0.82rem', lineHeight: '1.5', color: '#e2e8f0', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ marginTop: '8px', padding: '10px', backgroundColor: 'rgba(0, 240, 255, 0.05)', borderRadius: '10px', border: '1px solid var(--border-glass)' }}>
              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                Vollständige Rechtstexte sind auch im Projektordner unter <code>docs/PRIVACY_POLICY.md</code> und <code>docs/TERMS_OF_SERVICE.md</code> verfügbar.
              </span>
            </div>
          </div>
        )}
