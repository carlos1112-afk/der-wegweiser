...
// =============================================================================
// Agent Registry — imported from native PilotStudio aiAssistantService.ts
// All 16 agents across 6 providers
// =============================================================================
export type ProviderType = 'firebase-googleai' | 'vertexai' | 'openrouter' | 'huggingface' | 'pi' | 'apikey';
...
export const DEFAULT_AGENT = 'phi-pilot-fusion';

// =============================================================================
// PhiEngine — multi-provider AI engine for VS Code extension host
// =============================================================================
interface EnvKeys {
  huggingface?: string;
  gemini?: string;
...
export class PhiEngine {
  ...
  constructor() {
    ...
    const cfgAgent = config.get<string>('agent');
    if (cfgAgent && agentRegistry[cfgAgent]) {
      this._currentAgent = cfgAgent;
    }
  }

  get currentAgent(): string { return this._currentAgent; }
  get currentAgentEntry(): AgentEntry { return agentRegistry[this._currentAgent]; }

  public setAgent(id: string): boolean {
    if (!agentRegistry[id]) return false;
    this._currentAgent = id;
    vscode.workspace.getConfiguration('phiPilotFusion').update('agent', id, vscode.ConfigurationTarget.Global);
    return true;
  }

  public getAvailableAgents(): Array<{ id: string; entry: { label: string; description: string; tier: string; icon: string; provider: ProviderType; model: string }; available: boolean }> {
    const keys = this.loadKeysFromEnv();
    return Object.entries(agentRegistry).map(([id, entry]) => ({
      id,
  ...
  private isAgentAvailable(id: string, keys: EnvKeys): boolean {
  ...
  private loadKeysFromEnv(): EnvKeys {
    const keys: EnvKeys = {
      ...
      if (ws) {
        ...
        if (fs.existsSync(envPath)) {
          ...
          for (const line of content.split('\n')) {
            ...
            if (key === 'VITE_HUGGINGFACE_API_KEY') keys.huggingface = val;
            if (key === 'VITE_GEMINI_API_KEY') keys.gemini = val;
            if (key === 'VITE_OPENROUTER_API_KEY') keys.openrouter = val;
            if (key === 'VITE_PI_API_KEY') keys.pi = val;
            if (key === 'VITE_FIREBASE_API_KEY') keys.firebaseApiKey = val;
            if (key === 'VITE_GCP_PROJECT_ID' || key === 'GOOGLE_CLOUD_PROJECT' || key === 'GCP_PROJECT') keys.gcpProject = val;
            if (key === 'VITE_GCP_LOCATION') keys.gcpLocation = val;
          }
        }
      }

      // VS Code settings override .env.local and process environment.
      const cfgHF = config.get<string>('huggingfaceApiKey');
      if (cfgHF) keys.huggingface = cfgHF;
      const cfgGem = config.get<string>('geminiApiKey');
      if (cfgGem) keys.gemini = cfgGem;
      const cfgOR = config.get<string>('openrouterApiKey');
      if (cfgOR) keys.openrouter = cfgOR;
    } catch { /* keep environment fallback */ }
    return keys;
  }

  public validateConfig(): { ok: boolean; message: string } {
    const keys = this.loadKeysFromEnv();
    ...
    switch (agent.provider) {
      case 'openrouter':
        return this.callOpenAICompat(
          ...
          keys.openrouter!,
          agent.model, messages, systemInstruction, onChunk,
          { 'HTTP-Referer': 'https://github.com/pilotstudio', 'X-Title': 'Phi Pilot Fusion' }
        );
      case 'huggingface':
        return this.callOpenAICompat(
          'https://api-inference.huggingface.co/v1/chat/completions',
          keys.huggingface!, agent.model, messages, systemInstruction, onChunk
        );
      case 'pi':
        return this.callOpenAICompat(
          'https://api.inflection.ai/v1/chat/completions',
          keys.pi!, agent.model, messages, systemInstruction, onChunk
        );
      case 'vertexai':
  ...
  private callOpenAICompat(
    ...
    messages: ChatMessage[], systemInstruction: string | undefined,
    onChunk?: (c: string) => void, extraHeaders?: Record<string, string>
  ): Promise<string> {
    const apiMessages = [
    ...
    return this.httpPost(url, postData, headers, (data) => {
      ...
      if (errMsg) throw new Error(`${model}: ${errMsg}`);
  ...
  private callVertex(
    ...
    return new Promise((resolve, reject) => {
      execFile('gcloud', ['auth', 'print-access-token'], { windowsHide: true, timeout: 20_000 }, (error, stdout, stderr) => {
        if (error) {
  ...
  private callGeminiRequest(
    ...
    return this.httpPost(url, postData, headers, (data) => {
      ...
      if (errMsg) throw new Error(`Gemini ${model}: ${errMsg}`);
  ...
  private httpPost(
    ...
    const finish = (fn: () => void) => {
      if (!settled) { settled = true; if (timer) clearTimeout(timer); fn(); }
    };
